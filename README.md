...
- **Image optimization**: if `sharp` is installed, uploads are automatically converted to WebP and resized. If it's not installed, images are saved as-is and everything still works — you just don't get automatic conversion until you add it (see "Known limitations" below)
...
- **`sharp` (WebP conversion) is an optional dependency.** The code path is written and I was able to verify it works in the environment I built this in — but confirm it after your own `npm install` before assuming it's active in your deployment, since native image libraries can be finicky across different hosting platforms.
