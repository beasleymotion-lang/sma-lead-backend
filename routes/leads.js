...
router.get('/admin/leads', requireAuth, async (req, res) => {
  try {
    ...
    const leads = await crmDb.listLeads({
      stage,
      search
    });
...
router.put('/admin/leads/:id', requireAuth, async (req, res) => {
  try {
    const lead = await crmDb.updateLead(
...
router.get('/admin/leads-recent', requireAuth, async (req, res) => {
  try {
    const leads = await crmDb.recentLeads(5);
