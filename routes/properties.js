...
router.post('/admin/properties/:id/images', requireAuth, async (req, res) => {
  try {
    ...
    if (existing.length + uploads.length > 20) {
      return res.status(400).json({ ok: false, error: 'A listing can contain at most 20 photos.' });
    }

    const saved = await saveImages(uploads);
    const images = [...existing, ...saved].map((img, index) => ({ ...img, order: index }));
    const updated = await propertiesDb.updateProperty(id, { images });

    res.json({ ok: true, images: updated.images || images, property: updated });
  } catch (err) {
