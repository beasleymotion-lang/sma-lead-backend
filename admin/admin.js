...
function handleFiles(fileList) {
  ...
  if (remaining <= 0) { toast(`You can add up to ${MAX_LISTING_PHOTOS} photos per listing.`, true); return; }
  const files = [...fileList].filter(file => file.type.startsWith('image/')).slice(0, remaining);
  if ([...fileList].filter(file => file.type.startsWith('image/')).length > remaining) {
    toast(`Only ${MAX_LISTING_PHOTOS} photos are allowed per listing.`, true);
  }
  files.forEach((file) => {
    compressPhoto(file).then((dataUri) => {
      formImages.push({ dataUri, isNew: true, order: formImages.length });
      renderImageGrid();
    }).catch(() => toast(`Could not prepare ${file.name}.`, true));
  });
}
