...
let photoPreparationPromise = Promise.resolve();
...
function handleFiles(fileList) {
  ...
  // Queue photo preparation so Save cannot race the FileReader/canvas work.
  photoPreparationPromise = photoPreparationPromise.then(async () => {
    for (const file of files) {
      if (formImages.length >= MAX_LISTING_PHOTOS) break;
      try {
        const dataUri = await compressPhoto(file);
        formImages.push({ dataUri, isNew: true, order: formImages.length });
        renderImageGrid();
      } catch {
  ...
  // Allow selecting the same files again after this change event.
  const input = document.getElementById('file-input');
...
function renderImageGrid() {
  ...
  grid.innerHTML = formImages.map((img, i) => `
    <div class="image-tile ${i === 0 ? 'featured' : ''}" draggable="true" data-idx="${i}">
      <img src="${img.url || img.dataUri}" alt="">
...
function closePropertyForm() {
  ...
  photoPreparationPromise = Promise.resolve();
}

async function submitPropertyForm(e) {
  e.preventDefault();
  // Wait until every selected photo has finished browser-side compression.
  await photoPreparationPromise;
  const btn = document.getElementById('publish-btn');
  btn.disabled = true;
  const originalLabel = btn.textContent;

  const existingImages = formImages.filter((i) => !i.isNew).map((i, idx) => ({ ...i, order: idx }));
  const pendingImages = formImages.filter((i) => i.isNew);
  ...
  try {
    btn.textContent = pendingImages.length ? `Saving property · 0/${pendingImages.length} photos` : 'Saving property…';

    let property;
    if (editingPropertyId) {
      ({ property } = await apiFetch(`/api/admin/properties/${editingPropertyId}`, { method: 'PUT', body: JSON.stringify(payload) }));
    } else {
      ({ property } = await apiFetch('/api/admin/properties', { method: 'POST', body: JSON.stringify(payload) }));
    }

    // Upload new photos in small batches. This keeps each request comfortably
    // below Render's request-size limit while every actual file is stored in
    // Supabase Storage. Nothing permanent is written to Render.
    let uploadedImages = existingImages.slice();
    for (let start = 0; start < pendingImages.length; start += PHOTO_UPLOAD_BATCH_SIZE) {
      const batch = pendingImages.slice(start, start + PHOTO_UPLOAD_BATCH_SIZE).map(i => i.dataUri);
      const result = await apiFetch(`/api/admin/properties/${property.id}/images`, {
        method: 'POST',
        body: JSON.stringify({ newImageUploads: batch }),
      });
      uploadedImages = result.images || uploadedImages;
      btn.textContent = `Saving property · ${Math.min(start + batch.length, pendingImages.length)}/${pendingImages.length} photos`;
    }

    // Persist the exact order shown in the editor, including the new uploads.
    if (pendingImages.length) {
      const uploadedByUrl = new Map((uploadedImages || []).map(img => [img.url, img]));
      const pendingCount = pendingImages.length;
      const newUploaded = (uploadedImages || []).slice(-pendingCount);
      const finalImages = formImages.map((item, idx) => {
        if (!item.isNew) return { url: item.url, order: idx };
        const match = newUploaded.shift();
        return { ...(match || {}), url: match?.url || item.url, order: idx };
      }).filter(img => img.url);
...
// Patch openPropertyForm to render tag lists after DOM injection (defined above uses formTags directly)
const _rawOpenPropertyForm = openPropertyForm;
openPropertyForm = function (id) { _rawOpenPropertyForm(id); renderAllTagLists(); };
