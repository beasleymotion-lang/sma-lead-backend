...
let formImages = []; // {url, order} for existing + {dataUri, order} for new uploads pending save
    ...
    <form id="property-form">
      <div class="form-body">
        ...
        <div class="form-section">
          ...
          <div class="dropzone" id="dropzone">Drag &amp; drop photos here, or click to browse<br><span class="hint">First photo becomes the featured image. Drag tiles to reorder.</span></div>
...
function handleFiles(fileList) {
  [...fileList].forEach((file) => {
    ...
    reader.onload = () => {
      formImages.push({ dataUri: reader.result, isNew: true, order: formImages.length });
      renderImageGrid();
    };
...
async function submitPropertyForm(e) {
  ...
  const existingImages = formImages.filter((i) => !i.isNew).map((i, idx) => ({ url: i.url, order: idx }));
  const newImageUploads = formImages.filter((i) => i.isNew).map((i) => i.dataUri);
  ...
  try {
    if (editingPropertyId) {
      await apiFetch(`/api/admin/properties/${editingPropertyId}`, { method: 'PUT', body: JSON.stringify(payload) });
      toast('Property updated.');
    } else {
      await apiFetch('/api/admin/properties', { method: 'POST', body: JSON.stringify(payload) });
