...
function guideDeliveryEmail({ firstName, guideKey }) {
  ...
  const bodyHtml = `
    ...
    <p style="font-size:14px;color:${CHARCOAL2};line-height:1.7;margin:22px 0 8px;">Inside you'll learn:</p>
    <ul style="font-size:14px;color:${CHARCOAL2};line-height:1.9;padding-left:20px;margin:0 0 22px;">
      <li>Financing options</li>
      <li>Fideicomiso trusts</li>
      <li>Closing costs</li>
      <li>Neighborhood pricing</li>
      <li>Buying as a foreigner</li>
    </ul>
    <p style="font-size:15px;color:${CHARCOAL2};line-height:1.7;margin:0 0 16px;">
...
async function sendEmail(payload) {
  if (!process.env.RESEND_API_KEY) {
    console.warn('[email] RESEND_API_KEY not set — skipping send (dev mode). Payload:', payload.subject, '->', payload.to);
