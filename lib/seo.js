...
function listingAvailability(status) {
  if (status === 'sold') return 'https://schema.org/SoldOut';
  if (status === 'pending') return 'https://schema.org/PreOrder';
...
function generateStructuredData(p) {
  ...
  const listingUrl = propertyUrl(p.slug);
  return {
    ...
    ...(Number.isFinite(price) && price > 0 ? { offers: { '@type': 'Offer', price, priceCurrency: p.currency || 'USD', availability: listingAvailability(p.status), url: listingUrl } } : {}),
    ...
    broker: { '@type': 'Person', name: AGENT_NAME, url: SITE_URL }
  };
}
...
function generatePropertyKeywords(p) {
  return [p.property_type, p.neighborhood, 'San Miguel de Allende', p.status === 'for_rent' ? 'homes for rent' : 'homes for sale'].filter(Boolean).join(', ');
}
...
module.exports = { SITE_URL, AGENT_NAME, AGENCY, fmtPrice, propertyUrl, generateSeoTitle, generateMetaDescription, generateOgTags, generateStructuredData, generateBreadcrumbs, generatePropertyKeywords, listingAvailability, autofillSeoFields };
