...
function normalizeImages(value) {
  ...
  return raw
    .map((img) => {
      ...
      const url = img.url || img.publicUrl || img.public_url || '';
      return url ? { ...img, url } : null;
    })
    .filter((img) => img && /^https?:\/\//i.test(img.url))
    .filter((img) => !/\/uploads\//i.test(img.url) && !/\/tmp\/uploads\//i.test(img.url))
    .slice(0, 20)
...
async function listProperties({
  ...
  // The homepage's featured module may request featured for-sale inventory.
  // If the current featured inventory consists of rentals, don't let that
  // overly-specific filter make the public featured section appear empty.
  // Explicit search/filter requests remain unchanged whenever matching rows exist.
  if ((!data || data.length === 0) && featured && status === 'for_sale') {
    ...
    if (propertyType) fallback = fallback.eq('property_type', propertyType);
    if (minPrice != null) fallback = fallback.gte('price', minPrice);
    if (maxPrice != null) fallback = fallback.lte('price', maxPrice);
    if (bedrooms != null) fallback = fallback.gte('bedrooms', bedrooms);
    if (bathrooms != null) fallback = fallback.gte('bathrooms', bathrooms);
    ...
    switch (sort) {
      ...
      case 'price_high':
        ...
        break;
      case 'manual':
        fallback = fallback.order('sort_order', { ascending: true });
        break;
      default:
